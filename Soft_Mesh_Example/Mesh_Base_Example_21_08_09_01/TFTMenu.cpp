...
void TFTMenu::setup()
{
 ...
 // lcdOn(); // включаем подсветку
  ...
  //TFTPasswordScreen
  mbscrif.screen = new TFTPasswordScreen();
  mbscrif.screen->setup(this);
  mbscrif.screenName = "PASSWORD";
  screens.push_back(mbscrif);
...
// TFTPasswordScreen
//------------------------------------------------------------------------------------------------------------------------------------------------------------------------
TFTPasswordScreen::TFTPasswordScreen()
{
}
//------------------------------------------------------------------------------------------------------------------------------------------------------------------------
TFTPasswordScreen::~TFTPasswordScreen()
...
//------------------------------------------------------------------------------------------------------------------------------------------------------------------------
void TFTPasswordScreen::setup(TFTMenu* menuManager)
{
    TFT_Class* dc = menuManager->getDC();
   ...
   clearButton = screenButtons->addButton(left, top, button_width, button_height, CLEAR_CAPTION);
   top += button_height + v_spacing;
   backButton = screenButtons->addButton(left, top, button_width, button_height, WM_BACK_CAPTION);
   
}
//------------------------------------------------------------------------------------------------------------------------------------------------------------------------
void TFTPasswordScreen::update(TFTMenu* menuManager)
{

    // измерение закончено, можно проверять кнопки
    ...
    if(pressed_button != -1)
    {
        ...
        {
          if(enteredPass.length() < MAX_PASSWORD_LENGTH) // защита от длинного пароля
          {
            enteredPass += screenButtons->getLabel(pressed_button);
            screenButtons->relabelButton(textBox,enteredPass.c_str(),true);
          
            // Тут сравнение пароля
            if(ethalonPass.length() > 0 && ethalonPass.length() == enteredPass.length())
            {
              if(enteredPass == ethalonPass) // пароли совпадают
              {
...
//------------------------------------------------------------------------------------------------------------------------------------------------------------------------
void TFTPasswordScreen::onActivate(TFTMenu* menuManager)
{
  enteredPass = "";
  ethalonPass = Settings.GetPassword();
  screenButtons->relabelButton(textBox,enteredPass.c_str());
}
//------------------------------------------------------------------------------------------------------------------------------------------------------------------------
void TFTPasswordScreen::draw(TFTMenu* menuManager)
{
...
//------------------------------------------------------------------------------------------------------------------------------------------------------------------------
 ...
 void TFTServiceMenuScreen::setup(TFTMenu* menuManager)
 {
  ...
  cal_0_Button = screenButtons->addButton(left, top, button_width, button_height, "КАЛИБРОВКА 0%");
  top += v_spacing + button_height;

  cal_209_Button = screenButtons->addButton(left, top, button_width, button_height, "КАЛИБРОВКА 20.9%");
  top += v_spacing + button_height;

  measureTimeButton = screenButtons->addButton(left, top, button_width, button_height, "ВРЕМЯ ИЗМЕРЕНИЯ");
  top += v_spacing + button_height;

  calibrationTimeButton = screenButtons->addButton(left, top, button_width, button_height, "ВРЕМЯ КАЛИБРОВКИ");
  top += v_spacing + button_height;
...
//------------------------------------------------------------------------------------------------------------------------------------------------------------------------
 ...
 void TFTMenuScreen::setup(TFTMenu* menuManager)
 {
   ...
   int left = (screenWidth - BUTTON_WIDTH)/2;
   int top = (screenHeight - (BUTTON_HEIGHT*4 + V_SPACING))/2;

	 startButton = screenButtons->addButton(left, top, BUTTON_WIDTH, BUTTON_HEIGHT*3, START_CAPTION);
